...
from vngameengine import HSNeoOCIFolder, HSNeoOCI, HSNeoOCIProp, HSNeoOCILight, HSNeoOCIRoute, parseKeyCode
from vnactor import *
import vnframe
...
def scene_loaded(game):
    ...
    data,version = game.dataPNG_load("vnge_sss")
    if data != None:

        mas = _sc.loadFromStr(data)
        _sc.loadSceneDataInternalDict(mas,True)
...
class SceneConsole():
    def __init__(self, game):
        ...
        self.versionSaveInSceneData = "19.0"
        ...
        # :::: UI Data ::::
        self.consolenames = Array[String](("SceneSaveState", "Pose Library", "Scene Utils"))
    ...
    def addAutoWithMsg(self):
        self.addAuto()
        self.show_blocking_message_time_sc("Scene added!", 2.0)
    ...
    def loadSceneDataInternalDict(self,block_dict,file):
        if not block_dict == None:
    ...
    def exportToVNSS(self):
        scenesavestate_vnssext.exportToVNSS(self,{})
    ...
    def runVNSS(self, starfrom = "begin"):
        ...
        from skin_renpymini import SkinRenPyMini
        rpySkin = SkinRenPyMini()
...
class Scene():
    def __init__(self, actors=None, props=None, cams=None):
        self.cams = []
        self.actors = {}
        self.props = {}
    ...
    def copy(self):
        ...
        return Scene(json_decode(stractors),json_decode(strprops),json_decode(strcams))
    ...
    def setSceneState(self, game):
        ...
        for actid in self.actors:
            ...
            if actid == "sys":
                ...
                sys_import_status_diff_optimized(game, self.actors[actid])
            else:
...
def loadConfig():
    ...
    for command, key in config.items('Shortcuts'):
        ...
        """
        for com in _sc.game.gdata.sc_shortcuts.keys():
            if command == com.lower():
                _sc.game.gdata.sc_shortcuts[com][1] = parseKeyCode(key)
...
def hook_update(game, evid, u_param):
    if not game.gdata.hook_update_allowed:
        return
    ...
    if _sc.game.visible: # count only time when SSS is visible
    ...
    for commands, param in _sc.game.gdata.sc_shortcuts.items():
...
def setWindowName(index):
    ...
    if index in names.Keys:
        ...
        _sc.game.windowName = names[index]
    else:
...
def sceneConsoleTrackable():
    ...
    if not _sc.isSysTracking():
        ...
        if GUILayout.Button("UnTrack scene environment", GUILayout.Height(50), GUILayout.Width(160)):
            _sc.delSysTracking()
    GUILayout.EndHorizontal()
...
def sceneConsoleLdSvUI():
    ...
    if _sc.game.isSceneDataSaveSupported:
        ...
        GUILayout.FlexibleSpace()
        GUILayout.Label(
        "<color=#ff0000>!! (In Beta since 19.0) Data will save automatically in scene PNG\n(using standart Studio Save).\nAdditional save functions can be done just in backup purposes.</color>")
        GUILayout.FlexibleSpace()
        GUILayout.EndHorizontal()
        GUILayout.Space(20)
    ...
    _sc.vnFastIsRunImmediately = GUILayout.Toggle(_sc.vnFastIsRunImmediately, "And run from cur scene",
                                                  GUILayout.Height(20),
                                                  GUILayout.Width(210))
    GUILayout.FlexibleSpace()
...
def sceneConsoleEditUI():
    ...
    if len(_sc.block) > 0:
        for i in range(len(_sc.block)):
            if i == _sc.cur_index:
                ...
                col = _sc.nor_font_col
            scn_name = _sc.scene_str_array[i]
            ...
            if GUILayout.Button("<color=%s>%s</color>" % (col, scn_name)):
                _sc.cur_index = i
    ...
    if _sc.cur_index > -1:
        ...
        for i in range(0, len(_sc.block[_sc.cur_index].cams)):
            if i == _sc.cur_cam:
                col = _sc.sel_font_col
            else:
                col = "#f9f9f9"
    ...
    GUILayout.BeginVertical()
    if True:  # len(_sc.nameset[0])>0:
        _sc.fset_scroll = GUILayout.BeginScrollView(_sc.fset_scroll, GUILayout.Width(_sc.viewwidth),
                                                    GUILayout.Height(_sc.viewheight))
        for i in range(0, len(fset)):
            if i == _sc.fset_index:
                col = _sc.sel_font_col
            else:
                col = _sc.nor_font_col
            if GUILayout.Button("<color=%s>%s</color>" % (col, fset[i]), GUILayout.Height(40)):
                _sc.fset_index = i
        GUILayout.EndScrollView()
        # _sc.fset_index = GUILayout.SelectionGrid(_sc.fset_index,fset,1,GUILayout.Height(200))
        # if GUILayout.Button("Change FChar"):
        #     _sc.changeSceneChars(1, "upd")
        # if GUILayout.Button("Delete FChar"):
        #     if _sc.promptOnDelete:
        #         _sc.warning_param = (_sc.changeSceneChars, "Delete selected female character?", (1, "del"), False)
        #     else:
        #         _sc.changeSceneChars(1, "del")
    ...
    GUILayout.EndVertical()
    GUILayout.BeginVertical()
    if True:  # len(_sc.nameset[1])>0:
        _sc.mset_scroll = GUILayout.BeginScrollView(_sc.mset_scroll, GUILayout.Width(_sc.viewwidth),
                                                    GUILayout.Height(_sc.viewheight))
        for i in range(0, len(mset)):
            if i == _sc.mset_index:
                ...
                col = _sc.nor_font_col
            if GUILayout.Button("<color=%s>%s</color>" % (col, mset[i]), GUILayout.Height(40)):
                _sc.mset_index = i
        GUILayout.EndScrollView()
        # _sc.mset_index = GUILayout.SelectionGrid(_sc.mset_index,mset,1,GUILayout.Height(200))
        # if GUILayout.Button("Change MChar"):
        #     _sc.changeSceneChars(0, "upd")
        # if GUILayout.Button("Delete MChar"):
        #     if _sc.promptOnDelete:
        #         _sc.warning_param = (_sc.changeSceneChars, "Delete selected male character?", (0, "del"), False)
        #     else:
        #         _sc.changeSceneChars(0, "del")
    ...
    # _sc.addAuto(allbase = True)
    # if GUILayout.Button("Add dup prop folders"):
    # _sc.addPropFolders(dup=True)

    # if _sc.cur_index > -1 and GUILayout.Button("Designate PropFolder", GUILayout.Height(25)):
    #     _sc.addPropFolders()
    # if _sc.cur_index > -1 and GUILayout.Button("Designate Container", GUILayout.Height(25)):
    #     _sc.addProps()
    GUILayout.EndHorizontal()
    ...
    """
    GUILayout.BeginHorizontal()
    if GUILayout.Button("Load previous",GUILayout.Height(35)):
        _sc.goto_prev()
    if GUILayout.Button("Load next",GUILayout.Height(35)):
        _sc.goto_next()
    GUILayout.EndHorizontal()
    """
    GUILayout.Space(10)
...
# Minimize
def minimizeWindow():
    if _sc.game.windowRect.width > 200:
        _sc.consoleWidth = _sc.game.windowRect.width
        _sc.consoleHeight = _sc.game.windowRect.height
        _sc.game.windowRect.width = 120
        _sc.game.windowRect.height = 75
        ...
        _sc.game.skin.funcWindowGUI = sceneConsoleSkinWindowGUIMin
    else:
        _sc.game.windowRect.width = _sc.consoleWidth
        _sc.game.windowRect.height = _sc.consoleHeight
        _sc.game.skin.funcWindowGUI = sceneConsoleSkinWindowGUI
...
def sys_import_status_diff_optimized(game,status):
    ofs = export_sys_status(game)
    dfs = {}
    for key in status.Keys:
        if not key in ofs.Keys or ofs[key] != status[key]:
            dfs[key] = status[key]

    vnframe.act(game, {"sys":dfs})
...
def sceneUtilsUI():
    ...
    if(_sc.skinDefault_sideApp != "sceneutils"):
        _sc.skinDefault_sideApp = "sceneutils"
        import sceneutils
