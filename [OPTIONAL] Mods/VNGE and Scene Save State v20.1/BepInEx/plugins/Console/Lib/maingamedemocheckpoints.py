...
def start(game): #
    ...
    # we must init all chars here, because we don't know where checkpoints go to.
    ...
    game.gpersdata_load() # we need persistent data to handle checkpoints

    # setting list of checkpoints
    # 1 param - type. You can use different - "save", "gallery" to handle different sets of checkpoints
    game.checkpoint_set_list("save",[
        # array of checkpoints
        # each has set [checkId, calling goto function, additional params]
        # it's recommended to use ["textname"] as additional param - so, you can run auto menu making for them
        ["sc1", toSc1_0, ["First scene"]],
        ["sc11", (sup_load_checkpoint, ("scene1.png", toSc1_2)) , ["First part of first scene"]],
        ["sc12", (sup_load_checkpoint, ("scene1.png", toSc1_4)), ["Second part of first scene"]],
        ["sc2", toSc2_0, ["Second scene"]],
    ])

    game.set_text("s", "Main game demo with checkpoints")

    btns = ["New game", start2]
    if game.checkpoint_has_one("save"): # is there at least 1 save checkpoint?
        # show checkpoints menu
        btns += ["To last checkpoint", checkpoint_load_latest]
        btns += ["Checkpoint list", checkpoint_list]
        btns += ["(clean checkpoints)", checkpoint_clean]

    game.set_buttons_alt(btns)

def checkpoint_load_latest(game):
    """:type game: vngameengine.VNController"""
    game.checkpoint_goto_latest("save") # simplest variant

def checkpoint_clean(game):
    """:type game: vngameengine.VNController"""
    game.checkpoint_clean("save")
    start(game)

def checkpoint_list(game):
    """:type game: vngameengine.VNController"""
    btns = game.checkpoint_rendergotomenu("save", True)
    # if you want to hide non-passed checkpoints, use game.checkpoint_rendergotomenu("save", False)
    btns += ["<< return", start] # adding a button to return back
    game.set_text("s", "Choose checkpoint:")
    game.set_buttons_alt(btns,"compact")


def sup_load_checkpoint(game,param):
    # if we want to target middle of scene, we must load scene first...
    game.load_scene(param[0])

    game.set_timer(0.3,param[1]) # after loading scene - call needed function
...
def toSc1_0(game):
    """:type game: vngameengine.VNController"""
    game.checkpoint_save("save", "sc1") # passed 1 checkpoint. Params - type, checkId
...
def toSc1_2(game):
    ...
    game.checkpoint_save("save", "sc11")
    # ---------------------------
...
def toSc1_4(game):
    game.checkpoint_save("save", "sc12")  # passed 2 checkpoint
    game.to_camera(5)
...
def toSc2_0(game):
    """:type game: vngameengine.VNController"""
    game.checkpoint_save("save", "sc2")  # passed 2 checkpoint
